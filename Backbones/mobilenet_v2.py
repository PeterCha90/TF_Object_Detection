...
def inverted_res_block(inputs, expansion, out_channels,  
    ...
    x = layers.BatchNormalization(epsilon=1e-3, momentum=0.999,
                                  name='bn_conv_depthwise ' 
                                        + block_id)(x)
    x = tf.nn.relu6(x, name='conv_dw_relu ' + block_id)
    ...
    x = layers.BatchNormalization(epsilon=1e-3, momentum=0.999,
                                  name='bn_conv_bn_project ' 
                                        + block_id)(x)
    
    if in_channels == out_channels and stride == 1:
        return Add(name='res_connect_' + block_id)([inputs, x])
